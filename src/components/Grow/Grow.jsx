...
import { heroVectors, growCtaButtons } from "../../data/content";
  ...
  return (
    <section id="grow" className={styles.grow}>
      <div className={styles["grow__vectors"]} aria-hidden="true">
        {heroVectors.map((vector) => (
          <Image
            ...
            width={280}
            height={280}
            className={`${styles["grow__vector"]} ${styles[`grow__vector--${vector.modifier}`]}`}
            priority={false}
          />
        ))}
      </div>
      ...
      <div className={styles["grow__container"]}>
        <h2 className={styles["grow__headline"]}>Grow your business with AirWave</h2>
        <p className={styles["grow__description"]}>
          Discover the power of AirWave's marketing services and start earning today.
        </p>
